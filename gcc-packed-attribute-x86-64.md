...
這是最常見的用法。將 `packed` 屬性放在結構體定義的末尾，會使其效力遍及所有成員。編譯器會移除結構體內 所有 為了對齊而產生的 padding，並將每個成員（位元欄除外）的對齊要求降至 1-byte。如範例中的 `struct pxx` 所示，結構成員 `f`, `a`, `c`, `d`, `e` 皆緊密地排列在結構成員 `b` 之後。
